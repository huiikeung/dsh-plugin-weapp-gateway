// Persistent paired-device registry for weapp-gateway authentication.
//
...
function createRegistry(file, options = {}) {
  ...
  return {
    ...
    // A paired device can be relabelled later: the relay connector renames its
    // entry once the phone reports its own model, so the trusted-device list
    // shows "Xiaomi 14 Ultra" instead of the host running the connector.
    rename(deviceId, name) {
      const device = devices.find((candidate) => candidate.id === deviceId && !candidate.revokedAt)
      if (!device) return null
      if (typeof name !== 'string' || name.trim() === '') return null
      device.name = name.trim().slice(0, 80)
      save()
      return publicDevice(device, online.get(device.id) || 0)
    },
  }
}
