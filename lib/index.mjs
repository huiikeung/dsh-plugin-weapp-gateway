...
// dsh-plugin-weapp-gateway — persistent Host plugin.
//
// Two directions:
...
import { createRelayState, PAIRING_MODES } from './relay-state.mjs'
...
// Rename note: the package was renamed mobile-gateway -> weapp-gateway, but the
// on-disk paths below deliberately keep the old prefix. They hold a live
// deployment's state — the gateway identity and paired devices in
// `~/.dsh/mobile-gateway-devices.json`, the operator-written
// `/etc/dsh-mobile-gateway/public-url`, the helper socket and the log file — so
// renaming them would silently orphan an existing install and force re-pairing.
// Do not "clean these up" without a migration step.
...
const Config = Schema.object({
  ...
  relayStateFile: Schema.string().default(''),
...
const plugin = {
  ...
  apply(ctx, config) {
    ...
    if (typeof wsPath !== 'string' || !wsPath.startsWith('/') || wsPath.endsWith('/') || wsPath.includes('?') || wsPath.includes('#')) {
      throw new Error('weapp-gateway: path must be an absolute pathname without a trailing slash, query, or fragment')
    }
    if (options.lanEnabled === true && (typeof options.lanHost !== 'string' || !options.lanHost.trim())) {
      throw new Error('weapp-gateway: lanHost must be a non-empty listen address')
    }
    if (options.lanEnabled === true && options.lanAdvertiseHost && !isPrivateNetworkHostname(options.lanAdvertiseHost)) {
      throw new Error('weapp-gateway: lanAdvertiseHost must be localhost or a private LAN address')
    }
    ...
    const relayState = createRelayState(options.relayStateFile || `${deviceFile}.relay.json`)
        ...
        try {
          ...
          if (req.method === 'GET' && p === '/mgw/status') {
            sendJson(res, 200, {
              ...
              gatewayEnabled,
              gatewayMode,
              pairingMode: relayState.pairingMode,
          ...
          } else if (req.method === 'GET' && p === '/mgw/relay') {
            sendJson(res, 200, { pairingMode: relayState.pairingMode, relay: relayState.relay })
          } else if (req.method === 'POST' && p === '/mgw/relay') {
            // The fnOS connector registers its outbound-relay identity here so
            // the operator never copies a node id or public key by hand.
            const body = await readBody(req)
            try {
              relayState.setRelay({
                relay: String(body.relay || ''),
                nodeId: String(body.nodeId || ''),
                agentPubKey: String(body.agentPubKey || ''),
                gatewayName: String(body.gatewayName || gatewayIdentity.gatewayName).slice(0, 80),
              })
            } catch (error) {
              throw badRequest(`invalid relay registration: ${error.message}`)
            }
            log(`relay registered: nodeId=${relayState.relay.nodeId} via ${relayState.relay.relay}`)
            sendJson(res, 200, { pairingMode: relayState.pairingMode, relay: relayState.relay })
          } else if (req.method === 'POST' && p === '/mgw/mode') {
            const body = await readBody(req)
            if (!PAIRING_MODES.includes(body.mode)) throw badRequest(`mode must be one of ${PAIRING_MODES.join(', ')}`)
            relayState.setPairingMode(body.mode)
            log(`pairing mode changed to ${body.mode} from management UI`)
            sendJson(res, 200, { pairingMode: relayState.pairingMode, relay: relayState.relay })
            ...
            // In relay mode the phone must never be handed a direct gateway
            // address: the QR points at the relay and the connector dials the
            // gateway itself. Deriving gatewayUrl from the request would let the
            // editable "WebSocket 地址" field — which the fnOS runbook tells
            // operators to fill with a LAN address — leak into the payload and
            // send the mini-program straight at an endpoint it cannot
            // authenticate against.
            const advertisedGatewayUrl = normalizePublicUrl(configuredPublicUrl(), req, wsPath)
            // Two payload shapes behind one QR route: `direct` is the historical
            // one advertising this gateway's own endpoints; `relay` points the
            // mini-program at the outbound relay the fnOS connector holds open.
            const relay = relayState.relay
            const payload = relayState.pairingMode === 'relay' && relay
              ? {
                version: 3,
                mode: 'relay',
                relay: relay.relay,
          ...
          } else if (req.method === 'POST' && p.startsWith('/mgw/devices/') && p.endsWith('/rename')) {
            // Used by the relay connector: after pairing, the phone reports its
            // own model and the device is relabelled accordingly. Renaming is
            // cosmetic and scoped to one device, so an unknown id is a 404.
            const id = decodeURIComponent(p.slice('/mgw/devices/'.length, -'/rename'.length))
            const body = await readBody(req)
            const renamed = registry.rename(id, body.name)
            if (renamed) log(`device renamed: ${id} -> ${renamed.name}`)
            sendJson(res, renamed ? 200 : 404, { device: renamed })
    ...
    const installSessionQueueBaseline = (value) => {
      // DSH 0.1.6 dropped the queue channel from the session control baseline:
      // the baseline is now { jobs, projections } and the stream has no `queue`
      // frame type at all, while 0.1.5-rc.2 still sends `queues`. An absent
      // field therefore means "this Host has no queue channel", not a malformed
      // response — adopt an empty map once so clients see no queues instead of
      // failing the control stream and retrying it forever.
      if (value === undefined || value === null) {
        if (sessionQueues === null) {
          sessionQueues = new Map()
          log('session queue channel unavailable on this Host (no `queues` in session/control baseline); queues reported empty')
        }
        return
      }
