...
;(async () => {
  ...
  // field at all (and the stream has no `queue` frame type). The stream must
  // survive it — adopt an empty queue map — instead of failing and retrying
  // forever. Proof of survival: a later baseline and a later frame still land.
  controlFrames.push({ type: 'baseline', value: { jobs: {}, projections: {
    s1: { asOfSeq: 95, values: { todos: [], goal: null } },
  } } })
  const noQueuesBaselineSurvived = await waitFor(() => got.some(frame => frame.kind === 'projection-baseline' && frame.projections.s1?.asOfSeq === 95), 2000)
  controlFrames.push({ type: 'projection', sessionId: 's1', key: 'goal', value: null, seq: 96 })
  const frameAfterNoQueuesBaseline = await waitFor(() => got.some(frame => frame.kind === 'goal-updated' && frame.asOfSeq === 96 && frame.goal === null), 2000)
  interactionResults.push(['control baseline without queues keeps the stream alive', noQueuesBaselineSurvived && frameAfterNoQueuesBaseline])
